...
def process_sequence(capture, ref_id, input_path, conf, kind):
    ...
    keys = extract_keyframes(session=capture.sessions[sequence_id], conf=conf.matching)

    session_dict = {
        'session': capture.sessions[sequence_id],
        'device': kind,
        'session_id': sequence_id,
        'keys': keys
        }
    
    filename_keys = capture.session_path(sequence_id) / 'proc' / 'keyframes_original.txt'
    save_keyframes(session=session_dict, filename=filename_keys)
