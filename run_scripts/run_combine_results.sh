...
for scene in "${SCENES[@]}"; do
  ...
  for map_device in "${DEVICES[@]}"; do
    for query_device in "${DEVICES[@]}"; do
      ...
      file_path="${LOCATION_PATH}/${BENCHMARKING_DIR}/pose_estimation/${query_device}_query/${map_device}_map/superpoint/lightglue/netvlad-10/triangulation/${device_type}/poses.txt"
