...
for scene in "${SCENES[@]}"; do
  ...
  for map_device in "${DEVICES[@]}"; do
    for query_device in "${DEVICES[@]}"; do
      ...
      file_path="/data/capture_dir/${LOCATION_PATH}/${BENCHMARKING_DIR}/pose_estimation/${query_device}_query/${map_device}_map/${LOCAL_FEATURE_METHOD}/${MATCHING_METHOD}/${GLOBAL_FEATURE_METHOD}/triangulation/${device_type}/poses.txt"
