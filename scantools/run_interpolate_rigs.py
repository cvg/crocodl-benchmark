...
def run(session_path : Path,
    ...
    try: image_t = int(image_arr[0])
    except: image_t = MAX_INT

    try: depth_t = int(depth_arr[0])
    except: depth_t = MAX_INT

    try: imu_t = int(imu_arr[0])
    except: imu_t = MAX_INT

    try: trajectory_t = int(trajectory_arr[0])
    except: trajectory_t = MAX_INT
