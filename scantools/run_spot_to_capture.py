...
def run(input_path : Path, 
    ...
    elif not capture_path.is_dir():
        raise IOError(f"Output folder {capture_path} does not exist")
    ...
    additional_transforms_path = capture_path / "transforms.txt"
    ...
    for bag_path in bag_paths:
        ...
        if scene == "CAB":
            ...
            imu = None
            add_skip_topics = ["/zed2i/zed_node/depth/depth_registered", 
                               "/zed2i/zed_node/depth/camera_info", 
                               "/depth/image_rect",
                               "/depth/camera_info"]
            for topic in add_skip_topics:
                if topic not in skip_topics:
                    skip_topics.append(topic)
        ...
        if (capture_path / bag_name[:16]).is_dir() and overwrite == False:
            logger.info("Capture folder for bag exists and overwrite is False, skipping")
            continue
        ...
        run_nano_to_micro.run(capture_path=capture_path, 
...
if __name__ == "__main__":
    ...
    parser.add_argument("--capture_path", type=Path, required=True, help="Path of folder to place Capture sessions.")
